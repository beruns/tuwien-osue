...
/**
 * @brief creates a socket (SO_REUSEADDR), binds to int on port (AF_INET, INADDR_ANY), listens and returns the first client fd that connects
 * @param int sockfd Server socket's fd (will be set by function)
 * @param port Service to bind to
 * @return fd of connected client
 */
static int wait_for_client(int *sockfd, long int port);
 ...
 * @param buffer Data to write to client 
 * @param n Size to write (totally)
 * @return Pointer to buffer on success, else NULL
 */
static uint8_t *write_to_client(int fd, uint8_t *buffer, size_t n);
...
static int wait_for_client(int *sockfd, long int port) 
{
	...
	/* Bind socket to sockaddr_in info */
	if(bind(*sockfd, (struct sockaddr*)&saddr, saddr_size) < 0) {
		/* bail_out takes care of closing the socket */
	...
	/* return fd of first client to connect */
	return accept(*sockfd, (struct sockaddr*)&saddr,(socklen_t *)&saddr_size);
...
int main(int argc, char *argv[])
{
    ...
    */
    ...
    /* gh */
    if((connfd = wait_for_client(&sockfd, options.portno)) < 0) {
	bail_out(EXIT_FAILURE, "wait_for_client()");
    }
	...
	/* gh */
        if (write_to_client(connfd, buffer, WRITE_BYTES) == NULL) {
